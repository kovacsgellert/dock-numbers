#!/bin/bash
# Assemble dock-numbers.app from the SwiftPM release binary.
...
swift build -c release --product dock-numbers --package-path "$ROOT"
BIN="$(swift build -c release --show-bin-path --package-path "$ROOT")/dock-numbers"
